...
function g_table_bin_hdr(hParent)
...
if isfield(handles,'bg2')
    f2_pos = [f1_pos(1)+f1_pos(3)+16 f1_pos(2)-175 350  550];
elseif ~isfield(handles,'bg2')
    f2_pos = [f1_pos(1)+f1_pos(3)+16 f1_pos(2)-300 350  550];
end
f2 = figure('Name','Table bin header','NumberTitle','off',...
    'ToolBar','none','MenuBar', 'none','Resize','on',...
    'Position',f2_pos);

% t = uitable(f,'Data',bin_hdr,'Position',[0 0 400  400]);
handles.bin_hdr_table = uitable(f2,'Data',bin_hdr,'ColumnName',{'Description'; 'Start byte'; 'Length byte'; 'Value'},...
